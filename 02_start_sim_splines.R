...
different_y <- c(FALSE, TRUE) # for filenames
...
for (item_scen_a in seq_along(bin_items_a)) {
  ...
  for (item_scen in seq_along(bin_items)) {
    ...
    for (pop_scen in seq_along(a_list)) { # a_list elements are various densities for anchors
      for (test_scen in seq_along(y_list)) { # a_list elements are various densities for anchors
        ...
        res <- accuracy_indices(simres, list(true$true), true$W, "spline", method_names)
