...
generate_data_splines <- function(i, arg_list) {
  n <- arg_list[[1]]
  max_x <- arg_list$max$X
  max_y <- arg_list$max$Y
  max_a <- arg_list$max$A
  cor_xa <- arg_list[[3]][1] # correlation between X and A
  cor_ya <- arg_list[[3]][2]
  x_den <- arg_list[[4]]$X
  y_den <- arg_list[[4]]$Y
  xa_den <- arg_list[[4]]$XA
  ya_den <- arg_list[[4]]$YA
  poly_indices <- arg_list[[5]]
  w_x <- arg_list$w_x
  w_ax <- arg_list$w_ax
  w_y <- arg_list$w_y
  w_ay <- arg_list$w_ay
  ...
  colnames(x) <- paste(rep("X", ncol(x)), 1:ncol(x), sep = "")
  colnames(xa) <- paste(rep("A", ncol(xa)), 1:ncol(xa), sep = "")
  colnames(y) <- paste(rep("Y", ncol(y)), 1:ncol(y), sep = "")
  colnames(ya) <- paste(rep("A", ncol(ya)), 1:ncol(ya), sep = "")
